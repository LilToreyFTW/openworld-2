...
## Building interiors (C++ → WebAssembly for HTML game)

**All building interiors** (HUB tower 4km×2km 3 levels, houses, city buildings) are implemented in C++ and can run in the browser via WebAssembly.

| File | Purpose |
|------|--------|
| `InteriorGen.h` / `InteriorGen.cpp` | Interior layout generation: bounds, floor, tiles, ceiling, walls, windows, furniture, stairs. Outputs JSON for the frontend. |
| `InteriorGenWasm.cpp` | Emscripten bindings: `getInteriorLayoutJSON(buildingId, isHub, hubLevel, x, y, w, d, isHouse)` → JSON string. |
| `build_interior_wasm.bat` | Builds `interior_gen.js` + `interior_gen.wasm` (run from `cpp/` with Emscripten). |

**Build WASM (from `cpp/` with Emscripten installed):**

```bash
emsdk activate
cd cpp
build_interior_wasm.bat
```
...
emcc -o interior_gen.js InteriorGenWasm.cpp InteriorGen.cpp -I. \
  -s EXPORTED_RUNTIME_METHODS=ccall,cwrap -s MODULARIZE=1 -s EXPORT_NAME=InteriorGenModule \
  -s ALLOW_MEMORY_GROWTH=1 -std=c++17 -O2
```

The HTML game loads `interior-wasm.js`, which tries to load `cpp/interior_gen.js`. If the WASM module is present, all building interiors are generated by the C++ code; otherwise the game falls back to built-in JS generation.
